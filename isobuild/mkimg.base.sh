...
grub_gen_config() {
	...
	echo "terminal_output console gfxterm"

	echo "set theme=/boot/grub/theme.txt"
	...
	for _f in $kernel_flavors; do
		if [ -z "${xen_params+set}" ]; then
			...
			menuentry "Nightlight Cloud Xen/Linux $_f" {
...
profile_base() {
	...
	initfs_features="ata base bootchart cdrom dhcp ext4 mmc nvme raid scsi squashfs usb virtio plymouth"
	modloop_sign=yes
#	grub_mod="all_video disk part_gpt part_msdos linux normal configfile search search_label efi_gop fat iso9660 cat echo ls test true help gzio"
	grub_mod="all_video gfxterm gfxmenu png jpeg font disk part_gpt part_msdos linux normal configfile search search_label efi_gop efi_uga fat iso9660 cat echo ls test true help gzio"
	case "$ARCH" in
